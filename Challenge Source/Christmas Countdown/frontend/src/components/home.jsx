...
export function Home({ setPath }) {
  ...
  useEffect(() => {
    const token = Cookies.get('christmasCountdownToken');
    if (token) {
  ...
  async function updateCountdown(token=null) {
    ...
    if (token) {
      ...
      countdown = {
        ...
        day: 25,
        title: 'Christmas',
        description: 'Christmas is a festive holiday celebrated on December 25th!',
      };
  ...
  async function handleLogout() {
    Cookies.remove('christmasCountdownToken');
    setUsername(null);
    updateCountdown();
  }
